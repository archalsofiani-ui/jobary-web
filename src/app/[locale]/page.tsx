    ...
    <div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="bg-white rounded-2xl p-3 shadow-xl flex flex-col md:flex-row gap-3">
            <div className="flex-1 flex items-center gap-2 px-3">
              ...
              <input type="text" placeholder={t('search_placeholder')} className="w-full text-gray-900 text-sm focus:outline-none" />
            </div>
            <div className="hidden md:block w-px bg-gray-200" />
            <div className="flex-1 flex items-center gap-2 px-3">
              <MapPin className="h-5 w-5 text-gray-400 shrink-0" />
              <input type="text" placeholder={t('location_placeholder')} className="w-full text-gray-900 text-sm focus:outline-none" />
            </div>
            <Link href={`/${locale}/jobs`}>
              <Button size="lg" className="w-full md:w-auto rounded-xl whitespace-nowrap">{t('search_btn')}</Button>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {SECTORS.map(sector => (
            <Link key={sector.key} href={`/${locale}/jobs?sector=${sector.key}`}
              ...
              <span className="text-xs font-medium text-gray-700 group-hover:text-jobary-blue text-center">{ts(sector.key)}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="bg-jobary-light">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="bg-jobary-blue rounded-full p-3"><Briefcase className="h-6 w-6 text-white" /></div>
            <div>
          ...
          <Link href={`/${locale}/post-job`}>
            <Button size="lg" className="gap-2">{t('post_job_cta')} <ArrowRight className="h-4 w-4" /></Button>
          </Link>
        </div>
      </section>
    </div>
